        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          ...
          <div class="row placeholders">
            <div class="placeholder">
                  <a href="Purchase A Product.php#JumpToForms"><img src="Purchase A Product.jpg" width="700" height="100%" class="img-responsive" alt="Generic placeholder thumbnail" style="border-radius:30px;"></a>
                  <h4 id="JumpToForms">Our Purchases</h4>
                  <span class="text-muted">Products & Accessories</span>
                  <br/><br/><br/>
                  <h4>Have A Purchase</h4>
            </div>
            <div style="align: center; position : relative; right: 37.5%">
                 <form class="navbar-form navbar-right" style="position : relative;">
                      <table style="margin: 10px">
                      <div><input type = "button" class="form-control" value="Purchase Computers" onclick="window.location.href='Purchase A Computer.php'"/>
                      <div><br/><input type = "button" class="form-control" value="Purchase Accessories" onclick="window.location.href='Purchase An Accessory.php'"/>
                      </table>
                </form>
             </div>
          </div>
              ...
              <tbody>
                  <?php
                        ...
                        if(isset($_POST['search']))
                        {
                        	...
                        	$sql1 = "(SELECT DISTINCT Store_Name, Product_Name, Company_Name, Quantity, Price_per_Piece, Purchase_Date FROM Store AS s, Product AS p, Company AS c, Product_Purchase as pp WHERE pp.StoreID=s.StoreID AND pp.StoreID=s.StoreID AND pp.ProductID=p.ProductID AND pp.CompanyID=c.CompanyID AND (Store_Name LIKE '%$searchq%' OR Product_Name LIKE '%$searchq%' OR Company_Name LIKE '%$searchq%') ORDER BY Purchase_Date DESC) UNION (SELECT DISTINCT Store_Name, Accessory_Name, Company_Name, Quantity, Price_per_Piece, Purchase_Date FROM Store AS s, Accessory AS p, Company AS c, Accessory_Purchase as pp WHERE pp.StoreID=s.StoreID AND pp.AccessoryID=p.AccessoryID AND pp.CompanyID=c.CompanyID AND (Store_Name LIKE '%$searchq%' OR Accessory_Name LIKE '%$searchq%' OR Company_Name LIKE '%$searchq%') ORDER BY Purchase_Date DESC)" ;
                        }
                        else
                              $sql1 = "(SELECT DISTINCT Store_Name, Product_Name, Company_Name, Quantity, Price_per_Piece, Purchase_Date FROM Store AS s, Product AS p, Company AS c, Product_Purchase as pp WHERE pp.StoreID=s.StoreID AND pp.StoreID=s.StoreID AND pp.ProductID=p.ProductID AND pp.CompanyID=c.CompanyID ORDER BY Purchase_Date DESC) UNION (SELECT DISTINCT Store_Name, Accessory_Name, Company_Name, Quantity, Price_per_Piece, Purchase_Date FROM Store AS s, Accessory AS p, Company AS c, Accessory_Purchase as pp WHERE pp.StoreID=s.StoreID AND pp.AccessoryID=p.AccessoryID AND pp.CompanyID=c.CompanyID ORDER BY Purchase_Date DESC)";
